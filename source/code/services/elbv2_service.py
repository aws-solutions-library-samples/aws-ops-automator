...
LISTENER_CERTIFICATES = "ListenerCertificates"
LISTENERS = "Listeners"
...
CUSTOM_RESULT_PATHS = {
    ...
    LISTENER_CERTIFICATES: "Certificates",
...
class Elbv2Service(AwsService):
    ...
    def _get_tags_for_resource(self, client, resource):
        ...
        arn_name = self._resource_name[0:-1] + "Arn"
        ...
        if self._service_retry_strategy is not None:
            ...
            tag_list = client.describe_tags_with_retries(LoadBalancerNames=[resource[arn_name]]).get("TagDescriptions")

        else:
            tag_list = client.describe_tags(ResourceArns=[resource[arn_name]]).get("TagDescriptions")
